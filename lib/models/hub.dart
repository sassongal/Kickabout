...
class Hub with _$Hub {
  const factory Hub({
    ...
    Map<String, Timestamp> memberJoinDates, // userId -> join date timestamp
    @Default({
      'ratingMode': 'basic',
    ...
    @Default({})
    Map<String, String> roles, // userId -> role (manager, moderator, member)
    @Default({})
    Map<String, dynamic>
        permissions, // Custom permissions: {canCreateEvents: [userId1, userId2], canCreatePosts: [userId1, userId2]}
    ...
    @Default(false)
    bool
        isPrivate, // If true, requires "Request to Join" (create notification for manager)
    // Manager-only ratings for team balancing (1-10 scale)
    @Default({})
    Map<String, double>
        managerRatings, // userId -> rating (1-10, manager-only, for team balancing)
    // Payment settings
    String? paymentLink, // PayBox/Bit payment link URL
    // Denormalized fields (updated by Cloud Functions, not written by client)
    int?
        gameCount, // Denormalized: Total games created (updated by onGameCreated)
    @TimestampConverter()
    DateTime?
