...
/// Listens to domain events for real-time updates
class HubAnalyticsService {
  final FirebaseFirestore _firestore;
  final EventBus? _eventBus;
  StreamSubscription? _gameFinalizedSubscription;
  ...
  /// Setup event listeners for cross-feature notifications
  void _setupEventListeners() {
    ...
    _gameFinalizedSubscription = _eventBus.on<GameFinalizedEvent>().listen(
      ...
      onError: (e) => debugPrint('❌ Error handling GameFinalizedEvent: $e'),
    );

    // Listen for session ended events
    _sessionEndedSubscription = _eventBus.on<GameSessionEndedEvent>().listen(
      (event) => _onSessionEnded(event),
      onError: (e) => debugPrint('❌ Error handling GameSessionEndedEvent: $e'),
    );

    debugPrint('✅ HubAnalyticsService event listeners initialized');
  }

  /// Handle game finalized event
  Future<void> _onGameFinalized(GameFinalizedEvent event) async {
    if (event.hubId == null) return;

    try {
      debugPrint('📊 Updating hub analytics for finalized game: ${event.gameId}');

      // Update hub's last game date
      await _firestore.doc('hubs/${event.hubId}').update({
        'lastGameDate': FieldValue.serverTimestamp(),
        'totalGamesPlayed': FieldValue.increment(1),
        'updatedAt': FieldValue.serverTimestamp(),
      });

      debugPrint('✅ Hub analytics updated for game: ${event.gameId}');
    } catch (e) {
      debugPrint('❌ Failed to update hub analytics: $e');
    }
  }

  /// Handle session ended event
  Future<void> _onSessionEnded(GameSessionEndedEvent event) async {
    if (event.hubId == null) return;

    try {
      debugPrint('📊 Updating hub analytics for ended session: ${event.gameId}');

      // Update hub's session statistics
      await _firestore.doc('hubs/${event.hubId}').update({
        'totalSessionsPlayed': FieldValue.increment(1),
        'totalMatchesInSessions': FieldValue.increment(event.matchCount),
        'updatedAt': FieldValue.serverTimestamp(),
      });

      debugPrint('✅ Hub analytics updated for session: ${event.gameId}');
    } catch (e) {
      debugPrint('❌ Failed to update hub session analytics: $e');
    }
  }
  ...
  void dispose() {
    ...
    debugPrint('🧹 HubAnalyticsService event listeners disposed');
  }
